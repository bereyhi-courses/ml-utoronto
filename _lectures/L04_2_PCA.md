---
type: lecture
date: 2026-01-13T17:00:00
title: "Lecture 4 - Part II: PCA as Maximal Representation Variance"
tldr: "PCA"
stat: lec
# for lectures stat: lec
description: We show that the learning problem is reduced to minimal recovery error or equivalently maximal representation variance. The solution of this problem is given by principal components. 
videoID: 2muE5va3RPk 
hide_from_announcments: false
---
**Lecture Notes:**
- [Lecture 4]({{ site.baseurl }}/assets/Notes/Lecture4.pdf)

**Further Reads:**
* PCA, _Chapter 12_ of [[B]](https://www.bishopbook.com](https://www.microsoft.com/en-us/research/wp-content/uploads/2006/01/Bishop-Pattern-Recognition-and-Machine-Learning-2006.pdf)/)
* PCA, _Chapter 14_ of [[HTF]](https://hastie.su.domains/ElemStatLearn/)
* Latent Space, _Chapter 34_ of [[MK]](https://www.inference.org.uk/mackay/itila/book.html)
